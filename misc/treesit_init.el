...
;; ;; tree-sitter的颜色模式remap了mode的name。
;; (if (executable-find "clangd")
...
;; tree-sitter的颜色模式remap了mode的name。所以这里需要用c-ts-mode-hook
;; (add-hook 'c-ts-mode-hook 'lsp)
;; (add-hook 'c-ts-mode-hook 'helm-gtags-mode)
...
;; ;; tree-sitter 颜色模式
;; ;; 因为remap了，所以原有诸如 c-mode-hook 需要修改为c-ts-mode-hook
...
;; ;; 替换成功，但是颜色全是白色。
;; (define-derived-mode rustic-mode rust-ts-mode "R")
;; (dolist (ext '("\\.rs\\'"))
;;   (setf auto-mode-alist (assoc-delete-all ext auto-mode-alist))
;;   (add-to-list 'auto-mode-alist `(,ext . rustic-mode)))

;; (define-derived-mode python-mode python-ts-mode "Python")
;; (dolist (ext '("\\.py\\'"))
;;   (setf auto-mode-alist (assoc-delete-all ext auto-mode-alist))
;;   (add-to-list 'auto-mode-alist `(,ext . python-mode)))
